    ...
    for(int i=0; i<2000; i++){
        ...
        if(i%10 == 0){
            ...
            for (k = 0; k < POP_SIZE-1; k++){
                double min_fitness = population_fitness[k];
                int mfi = k;

                for (j = k+1; j < POP_SIZE; j++) {
                ...
                population_fitness[mfi] = population_fitness[k];
                population[mfi] = population[k];
                population_fitness[k] = min_fitness;
                population[k] = aux;
            }

            for (size_t i = 0; i < LS_NUM; i++) {
                j = POP_SIZE-i-1;
                population[j] = findLocalSearchSolution(population[j]);
                population_fitness[j] = evaluation(population[j]);
            }
